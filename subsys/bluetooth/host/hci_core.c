...
struct bt_dev *bt_dev_alloc(uint8_t dev_id)
{
...
#if DT_HAS_CHOSEN(zephyr_bt_hci)
static bool drv_quirk_no_reset(struct bt_dev *hdev)
{
...
static bool drv_quirk_no_reset(struct bt_dev *hdev)
{
	return  ((hdev->drv->quirks & BT_QUIRK_NO_RESET) != 0);
}

bool bt_drv_quirk_no_auto_dle(struct bt_dev *hdev)
{
	return  ((hdev->drv->quirks & BT_QUIRK_NO_AUTO_DLE) != 0);
}
#endif
...
void bt_send_one_host_num_completed_packets(struct bt_dev *hdev, uint16_t handle)
{
	...
	err = bt_hci_cmd_send(hdev, BT_HCI_OP_HOST_NUM_COMPLETED_PACKETS, buf);
...
void bt_hci_host_num_completed_packets(struct net_buf *buf)
{
	...
	struct bt_dev *hdev = acl(buf)->hdev;
	...
	if (!BT_CMD_TEST(hdev->supported_commands, 10, 5)) {
		return;
	}

	conn = bt_conn_lookup_index(hdev, index);
	if (!conn) {
	...
	bt_send_one_host_num_completed_packets(hdev, handle);
}
...
int bt_hci_le_read_max_data_len(struct bt_dev *hdev, uint16_t *tx_octets, uint16_t *tx_time)
{
	...
	err = bt_hci_cmd_send_sync(hdev, BT_HCI_OP_LE_READ_MAX_DATA_LEN, NULL, &rsp);
	if (err) {
...
static void hci_acl(struct bt_dev *hdev, struct net_buf *buf)
{
	...
	conn = bt_conn_lookup_handle(hdev, acl(buf)->handle, BT_CONN_TYPE_ALL);
	if (!conn) {
...
int bt_le_create_conn_ext(const struct bt_conn *conn)
{
	...
	err = bt_id_set_create_conn_own_addr(hdev, use_filter, &own_addr_type);
	if (err) {
		return err;
	}

	num_phys = (!(hdev->create_param.options &
		      BT_CONN_LE_OPT_NO_1M) ? 1 : 0) +
		   ((hdev->create_param.options &
	...
	if (use_filter) {
		...
		if (hdev->le.rl_entries > hdev->le.rl_size) {
	...
	if (!(hdev->create_param.options & BT_CONN_LE_OPT_NO_1M)) {
		cp->phys |= BT_HCI_LE_EXT_SCAN_PHY_1M;
		phy = net_buf_add(buf, sizeof(*phy));
		phy->scan_interval = sys_cpu_to_le16(
			hdev->create_param.interval);
		phy->scan_window = sys_cpu_to_le16(
			hdev->create_param.window);
		set_phy_conn_param(conn, phy);
	}

	if (hdev->create_param.options & BT_CONN_LE_OPT_CODED) {
		cp->phys |= BT_HCI_LE_EXT_SCAN_PHY_CODED;
		phy = net_buf_add(buf, sizeof(*phy));
		phy->scan_interval = sys_cpu_to_le16(
			hdev->create_param.interval_coded);
		phy->scan_window = sys_cpu_to_le16(
			hdev->create_param.window_coded);
		set_phy_conn_param(conn, phy);
	}

	bt_hci_cmd_state_set_init(buf, &state, hdev->flags,
				  BT_DEV_INITIATING, true);

	return bt_hci_cmd_send_sync(hdev, BT_HCI_OP_LE_EXT_CREATE_CONN, buf, NULL);
}
			     ...
			     uint8_t subevent)
{
	...
	err = bt_id_set_create_conn_own_addr(hdev, false, &own_addr_type);
	if (err) {
		return err;
	}
	...
	bt_hci_cmd_state_set_init(buf, &state, hdev->flags, BT_DEV_INITIATING, true);

	return bt_hci_cmd_send_sync(hdev, BT_HCI_OP_LE_EXT_CREATE_CONN_V2, buf, NULL);
}
...
static int bt_le_create_conn_legacy(const struct bt_conn *conn)
{
	...
	cp->scan_interval = sys_cpu_to_le16(hdev->create_param.interval);
	cp->scan_window = sys_cpu_to_le16(hdev->create_param.window);
	...
	return bt_hci_cmd_send_sync(conn->hdev, BT_HCI_OP_LE_CREATE_CONN, buf, NULL);
}

int bt_le_create_conn(const struct bt_conn *conn)
{
	if (IS_ENABLED(CONFIG_BT_EXT_ADV) &&
	    BT_DEV_FEAT_LE_EXT_ADV(conn->hdev->le.features)) {
...
int bt_le_create_conn_cancel(struct bt_dev *hdev)
{
	...
	return bt_hci_cmd_send_sync(hdev, BT_HCI_OP_LE_CREATE_CONN_CANCEL, buf, NULL);
}
...
int bt_hci_disconnect(struct bt_dev *hdev, uint16_t handle, uint8_t reason)
{
	...
	return bt_hci_cmd_send_sync(hdev, BT_HCI_OP_DISCONNECT, buf, NULL);
}

static void disconnected_handles_reset(struct bt_dev *hdev)
{
	(void)memset(hdev->disconnected_handles, 0, sizeof(hdev->disconnected_handles));
}

static void conn_handle_disconnected(struct bt_dev *hdev, uint16_t handle, uint8_t disconnect_reason)
{
	for (int i = 0; i < ARRAY_SIZE(hdev->disconnected_handles); i++) {
		if (!hdev->disconnected_handles[i]) {
			/* Use invalid connection handle bits so that connection
			 * handle 0 can be used as a valid non-zero handle.
			 */
			hdev->disconnected_handles[i] = ~BT_ACL_HANDLE_MASK | handle;
			hdev->disconnected_handles_reason[i] = disconnect_reason;
		}
	}
}

/** @returns the disconnect reason. */
static uint8_t conn_handle_is_disconnected(struct bt_dev *hdev, uint16_t handle)
{
	handle |= ~BT_ACL_HANDLE_MASK;

	for (int i = 0; i < ARRAY_SIZE(hdev->disconnected_handles); i++) {
		if (hdev->disconnected_handles[i] == handle) {
			hdev->disconnected_handles[i] = 0;
			return hdev->disconnected_handles_reason[i];
		}
	}
...
static void hci_disconn_complete_prio(struct bt_dev *hdev, struct net_buf *buf)
{
	...
	if (!conn) {
		 ...
		 */
		conn_handle_disconnected(hdev, handle, evt->reason);
		return;
	}
...
static void hci_disconn_complete(struct bt_dev *hdev, struct net_buf *buf)
{
	...
	if (atomic_test_bit(conn->flags, BT_CONN_AUTO_CONNECT)) {
		...
		int err = bt_le_scan_user_remove(hdev, BT_LE_SCAN_USER_NONE);
...
int bt_hci_read_remote_version(struct bt_conn *conn)
{
	...
	return bt_hci_cmd_send_sync(conn->hdev, BT_HCI_OP_READ_REMOTE_VERSION_INFO, buf,
				    NULL);
}
...
int bt_le_set_data_len(struct bt_conn *conn, uint16_t tx_octets, uint16_t tx_time)
{
	...
	return bt_hci_cmd_send_sync(conn->hdev, BT_HCI_OP_LE_SET_DATA_LEN, buf, NULL);
}
...
static int hci_le_read_phy(struct bt_conn *conn)
{
	...
	err = bt_hci_cmd_send_sync(conn->hdev, BT_HCI_OP_LE_READ_PHY, buf, &rsp);
	if (err) {
		return err;
	}
...
int bt_le_read_conn_rssi(struct bt_conn *conn, int8_t *rssi)
{
	...
	err = bt_hci_cmd_send_sync(conn->hdev, BT_HCI_OP_READ_RSSI, buf, &rsp);
	if (err) {
		return err;
	}
...
static struct bt_conn *find_pending_connect(struct bt_dev *hdev, uint8_t role, bt_addr_le_t *peer_addr)
{
	struct bt_conn *conn;
	...
	if (IS_ENABLED(CONFIG_BT_CENTRAL) && role == BT_HCI_ROLE_CENTRAL) {
		conn = bt_conn_lookup_state_le(hdev, BT_ID_DEFAULT, peer_addr,
					       BT_CONN_INITIATING);
		if (IS_ENABLED(CONFIG_BT_FILTER_ACCEPT_LIST) && !conn) {
			conn = bt_conn_lookup_state_le(hdev, BT_ID_DEFAULT,
						       BT_ADDR_LE_NONE,
	...
	if (IS_ENABLED(CONFIG_BT_PERIPHERAL) && role == BT_HCI_ROLE_PERIPHERAL) {
		conn = bt_conn_lookup_state_le(hdev, hdev->adv_conn_id, peer_addr,
					       BT_CONN_ADV_DIR_CONNECTABLE);
		if (!conn) {
			conn = bt_conn_lookup_state_le(hdev, hdev->adv_conn_id,
						       BT_ADDR_LE_NONE,
...
static void le_conn_complete_cancel(struct bt_dev *hdev, uint8_t err)
{
	int ret;
	struct bt_conn *conn;
	 ...
	 */
	conn = find_pending_connect(hdev, BT_HCI_ROLE_CENTRAL, NULL);
	if (!conn) {
...
static void le_conn_complete_adv_timeout(struct bt_dev *hdev)
{
	if (!(IS_ENABLED(CONFIG_BT_EXT_ADV) &&
		...
		if (IS_ENABLED(CONFIG_BT_EXT_ADV) &&
			 ...
			 */
			bt_le_adv_delete_legacy(hdev);
		}

		/* There is no need to check ID address as only one
		 * connection in peripheral role can be in pending state.
		 */
		conn = find_pending_connect(hdev, BT_HCI_ROLE_PERIPHERAL, NULL);
		if (!conn) {
...
static void enh_conn_complete(struct bt_dev *hdev,
...
void bt_hci_le_enh_conn_complete(struct bt_dev *hdev,
			 struct bt_hci_evt_le_enh_conn_complete *evt)
{
	__ASSERT_NO_MSG(evt->status == BT_HCI_ERR_SUCCESS);

	uint16_t handle = sys_le16_to_cpu(evt->handle);
	uint8_t disconnect_reason = conn_handle_is_disconnected(hdev, handle);
	...
	if (!conn) {
		...
		bt_hci_disconnect(hdev, handle, BT_HCI_ERR_REMOTE_USER_TERM_CONN);
		return;
	}
	...
	if (IS_ENABLED(CONFIG_BT_CENTRAL) && conn->role == BT_HCI_ROLE_CENTRAL) {
		...
		/* Just a best-effort check if the scanner should be started. */
		err = bt_le_scan_user_remove(hdev, BT_LE_SCAN_USER_NONE);
		if (err) {
			LOG_WRN("Error while updating the scanner (%d)", err);
		}
	}
				      ...
				      struct bt_le_per_adv_sync *sync)
{
	...
	conn = bt_conn_add_le(hdev, BT_ID_DEFAULT, BT_ADDR_LE_ANY);
	if (!conn) {
...
static void enh_conn_complete_error_handle(struct bt_dev *hdev, uint8_t status)
{
	if (IS_ENABLED(CONFIG_BT_PERIPHERAL) && status == BT_HCI_ERR_ADV_TIMEOUT) {
		le_conn_complete_adv_timeout(hdev);
		return;
	}

	if (IS_ENABLED(CONFIG_BT_CENTRAL) && status == BT_HCI_ERR_UNKNOWN_CONN_ID) {
		le_conn_complete_cancel(hdev, status);
		int err = bt_le_scan_user_remove(hdev, BT_LE_SCAN_USER_NONE);
...
static void le_enh_conn_complete(struct bt_dev *hdev, struct net_buf *buf)
{
	...
	if (evt->status != BT_HCI_ERR_SUCCESS) {
		enh_conn_complete_error_handle(hdev, evt->status);
		return;
	}
...
static void le_legacy_conn_complete(struct bt_dev *hdev, struct net_buf *buf)
{
	...
	if (IS_ENABLED(CONFIG_BT_PRIVACY)) {
		bt_addr_copy(&enh.local_rpa, &hdev->random_addr.a);
	} else {
...
static void le_conn_param_neg_reply(struct bt_dev *hdev, uint16_t handle, uint8_t reason)
{
	...
	bt_hci_cmd_send(hdev, BT_HCI_OP_LE_CONN_PARAM_REQ_NEG_REPLY, buf);
}

static int le_conn_param_req_reply(struct bt_dev *hdev, uint16_t handle,
				   const struct bt_le_conn_param *param)
{
	...
	return bt_hci_cmd_send(hdev, BT_HCI_OP_LE_CONN_PARAM_REQ_REPLY, buf);
}
...
static void le_conn_param_req(struct bt_dev *hdev, struct net_buf *buf)
{
	...
	if (!conn) {
		LOG_ERR("Unable to lookup conn for handle %u", handle);
		le_conn_param_neg_reply(hdev, handle, BT_HCI_ERR_UNKNOWN_CONN_ID);
		return;
	}

	if (!le_param_req(conn, &param)) {
		le_conn_param_neg_reply(hdev, handle, BT_HCI_ERR_INVALID_LL_PARAM);
	} else {
		le_conn_param_req_reply(hdev, handle, &param);
	}
...
#if defined(CONFIG_BT_HCI_ACL_FLOW_CONTROL)
static int set_flow_control(struct bt_dev *hdev)
{
	...
	return bt_hci_cmd_send_sync(hdev, BT_HCI_OP_SET_CTL_TO_HOST_FLOW, buf, NULL);
}
#endif /* CONFIG_BT_HCI_ACL_FLOW_CONTROL */

static void unpair(struct bt_dev *hdev, uint8_t id, const bt_addr_le_t *addr)
{
	struct bt_keys *keys = NULL;
	struct bt_conn *conn = bt_conn_lookup_addr_le_mc(hdev->dev_id, id, addr);
	...
	if (IS_ENABLED(CONFIG_BT_CLASSIC)) {
		...
		if (addr->type == BT_ADDR_LE_PUBLIC) {
			bt_keys_link_key_clear_addr(hdev, &addr->a);
		}
	}

	if (IS_ENABLED(CONFIG_BT_SMP)) {
		if (!keys) {
			keys = bt_keys_find_addr(hdev, id, addr);
		}
...
{
	struct unpair_data *udata = (struct unpair_data *) data;

	unpair(udata->hdev, udata->id, &info->addr);
}

int bt_unpair_mc(uint8_t dev_id, uint8_t id, const bt_addr_le_t *addr)
{
	struct bt_dev *hdev = bt_dev_get(dev_id);
	struct unpair_data data = {
		.id = id,
		.hdev = hdev,
	};
	...
	if (IS_ENABLED(CONFIG_BT_SMP)) {
		if (!addr || bt_addr_le_eq(addr, BT_ADDR_LE_ANY)) {
			bt_foreach_bond_mc(dev_id, id, unpair_remote, &data);
		} else {
			unpair(hdev, id, addr);
		}
	} else {
...
#if defined(CONFIG_BT_SMP)
static void le_ltk_neg_reply(struct bt_dev *hdev, uint16_t handle)
{
	...
	bt_hci_cmd_send(hdev, BT_HCI_OP_LE_LTK_REQ_NEG_REPLY, buf);
}

static void le_ltk_reply(struct bt_dev *hdev, uint16_t handle, uint8_t *ltk)
{
...
static void le_ltk_request(struct bt_dev *hdev, struct net_buf *buf)
{
	...
	if (bt_smp_request_ltk(conn, evt->rand, evt->ediv, ltk)) {
		le_ltk_reply(hdev, handle, ltk);
	} else {
		le_ltk_neg_reply(hdev, handle);
	}
...
static void hci_cmd_done(struct bt_dev *hdev, uint16_t opcode,
			 uint8_t status, struct net_buf *evt_buf)
{
...
#if defined(CONFIG_BT_HCI_VS_EVT_USER)
	if (hdev->hci_vnd_evt_cb) {
		struct net_buf_simple_state state;

		net_buf_simple_save(&buf->b, &state);

		handled = hdev->hci_vnd_evt_cb(&buf->b);
...
static void hci_core_send_cmd(struct bt_dev *hdev)
{
	...
	/* Get next command */
	LOG_DBG("dev:%d, fetch cmd", hdev->dev_id);
...
static void read_buffer_size_complete(struct bt_dev *hdev, struct net_buf *buf)
{
	...
	hdev->le.acl_mtu = sys_le16_to_cpu(rp->acl_max_len);
	pkts = sys_le16_to_cpu(rp->acl_max_num);

	LOG_DBG("ACL BR/EDR buffers: pkts %u mtu %u", pkts, hdev->le.acl_mtu);

	k_sem_init(&hdev->le.acl_pkts, pkts, pkts);
}
#endif /* !defined(CONFIG_BT_CLASSIC) */
#endif /* CONFIG_BT_CONN */

static void le_read_buffer_size_complete(struct bt_dev *hdev, struct net_buf *buf)
{
	...
	hdev->le.acl_mtu = acl_mtu;

	LOG_DBG("ACL LE buffers: pkts %u mtu %u", rp->le_max_num, hdev->le.acl_mtu);

	k_sem_init(&hdev->le.acl_pkts, rp->le_max_num, rp->le_max_num);
#endif /* CONFIG_BT_CONN */
}

static void read_buffer_size_v2_complete(struct bt_dev *hdev, struct net_buf *buf)
{
#if defined(CONFIG_BT_ISO)
	...
	if (acl_mtu && rp->acl_max_num) {
		hdev->le.acl_mtu = acl_mtu;
		LOG_DBG("ACL LE buffers: pkts %u mtu %u", rp->acl_max_num, hdev->le.acl_mtu);

		k_sem_init(&hdev->le.acl_pkts, rp->acl_max_num, rp->acl_max_num);
	}
#endif /* CONFIG_BT_CONN */
	...
	hdev->le.iso_mtu = iso_mtu;

	LOG_DBG("ISO buffers: pkts %u mtu %u", rp->iso_max_num, hdev->le.iso_mtu);

	k_sem_init(&hdev->le.iso_pkts, rp->iso_max_num, rp->iso_max_num);
	hdev->le.iso_limit = rp->iso_max_num;
#endif /* CONFIG_BT_ISO */
}
...
static int le_set_host_feature(struct bt_dev *hdev, uint8_t bit_number, uint8_t bit_value)
{
	...
	return bt_hci_cmd_send_sync(hdev, BT_HCI_OP_LE_SET_HOST_FEATURE, buf, NULL);
}
...
static void read_supported_commands_complete(struct bt_dev *hdev, struct net_buf *buf)
{
	...
	memcpy(hdev->supported_commands, rp->commands,
...
#if defined(CONFIG_BT_BROADCASTER)
static void le_read_maximum_adv_data_len_complete(struct bt_dev *hdev, struct net_buf *buf)
{
	struct bt_hci_rp_le_read_max_adv_data_len *rp = (void *)buf->data;

	LOG_DBG("status 0x%02x %s", rp->status, bt_hci_err_to_str(rp->status));

	hdev->le.max_adv_data_len = sys_le16_to_cpu(rp->max_adv_data_len);
}
#endif /* CONFIG_BT_BROADCASTER */

#if defined(CONFIG_BT_SMP)
static void le_read_resolving_list_size_complete(struct bt_dev *hdev, struct net_buf *buf)
{
	struct bt_hci_rp_le_read_rl_size *rp = (void *)buf->data;

	LOG_DBG("Resolving List size %u", rp->rl_size);

	hdev->le.rl_size = rp->rl_size;
}
...
static int le_set_event_mask(struct bt_dev *hdev)
{
	...
	if (IS_ENABLED(CONFIG_BT_CONN)) {
		if ((IS_ENABLED(CONFIG_BT_SMP) &&
		     BT_FEAT_LE_PRIVACY(hdev->le.features)) ||
		    (IS_ENABLED(CONFIG_BT_EXT_ADV) &&
		     BT_DEV_FEAT_LE_EXT_ADV(hdev->le.features))) {
			/* C24:
		...
		if (IS_ENABLED(CONFIG_BT_DATA_LEN_UPDATE) &&
		    BT_FEAT_LE_DLE(hdev->le.features)) {
			mask |= BT_EVT_MASK_LE_DATA_LEN_CHANGE;
		}

		if (IS_ENABLED(CONFIG_BT_PHY_UPDATE) &&
		    (BT_FEAT_LE_PHY_2M(hdev->le.features) ||
	...
	if (IS_ENABLED(CONFIG_BT_SMP) &&
	    BT_FEAT_LE_ENCR(hdev->le.features)) {
	...
	if (IS_ENABLED(CONFIG_BT_ECC) &&
	    (BT_CMD_TEST(hdev->supported_commands, 34, 1)) &&
	    (BT_CMD_TEST(hdev->supported_commands, 34, 2))) {
	...
	if (IS_ENABLED(CONFIG_BT_ISO) &&
	    BT_FEAT_LE_CIS(hdev->le.features)) {
...
static int le_init_iso(struct bt_dev *hdev)
{
	...
	if (BT_CMD_TEST(hdev->supported_commands, 41, 5)) {
		...
		err = bt_hci_cmd_send_sync(hdev, BT_HCI_OP_LE_READ_BUFFER_SIZE_V2,
					   NULL, &rsp);
		if (err) {
			return err;
		}

		read_buffer_size_v2_complete(hdev, rsp);
		 ...
		 */
		err = bt_hci_cmd_send_sync(hdev, BT_HCI_OP_LE_READ_BUFFER_SIZE,
					   NULL, &rsp);
		if (err) {
			return err;
		}

		le_read_buffer_size_complete(hdev, rsp);
...
static int le_init(struct bt_dev *hdev)
{
	...
	err = bt_hci_cmd_send_sync(hdev, BT_HCI_OP_LE_READ_LOCAL_FEATURES, NULL,
				   &rsp);
	if (err) {
		return err;
	}
	...
	if (IS_ENABLED(CONFIG_BT_EXT_ADV) && BT_DEV_FEAT_LE_EXT_ADV(hdev->le.features)) {
		...
		err = bt_hci_cmd_send_sync(hdev, BT_HCI_OP_LE_READ_MAX_ADV_DATA_LEN, NULL, &rsp);
		if (err == 0) {
			le_read_maximum_adv_data_len_complete(hdev, rsp);
			net_buf_unref(rsp);
		} else if (err == -EIO) {
	...
	if (BT_FEAT_BREDR(hdev->features)) {
		...
		cp_le->le = 0x01;
		cp_le->simul = 0x00;
		err = bt_hci_cmd_send_sync(hdev, BT_HCI_OP_LE_WRITE_LE_HOST_SUPP, buf,
					   NULL);
		if (err) {
			return err;
		}
	...
	if (BT_CMD_LE_STATES(hdev->supported_commands)) {
		err = bt_hci_cmd_send_sync(hdev, BT_HCI_OP_LE_READ_SUPP_STATES, NULL,
					   &rsp);
		if (err) {
			return err;
		}
	...
	if (IS_ENABLED(CONFIG_BT_CONN) &&
		...
		err = bt_hci_le_read_max_data_len(hdev, &tx_octets, &tx_time);
		if (err) {
			return err;
		}
		...
		err = bt_hci_cmd_send_sync(hdev, BT_HCI_OP_LE_WRITE_DEFAULT_DATA_LEN,
					   buf, NULL);
		if (err) {
			return err;
		}
	...
	if (BT_FEAT_LE_PRIVACY(hdev->le.features)) {
		...
		err = bt_hci_cmd_send_sync(hdev, BT_HCI_OP_LE_SET_RPA_TIMEOUT, buf,
					   NULL);
		if (err) {
			return err;
		}
		...
		err = bt_hci_cmd_send_sync(hdev, BT_HCI_OP_LE_READ_RL_SIZE, NULL,
					   &rsp);
		if (err) {
			return err;
		}
		le_read_resolving_list_size_complete(hdev, rsp);
		net_buf_unref(rsp);
	}
#endif
...
static int bt_br_init(struct bt_dev *hdev)
{
	...
	read_buffer_size_complete(hdev, rsp);
	net_buf_unref(rsp);
#endif /* CONFIG_BT_CONN */
...
static int set_event_mask(struct bt_dev *hdev)
{
	...
	return bt_hci_cmd_send_sync(hdev, BT_HCI_OP_SET_EVENT_MASK, buf, NULL);
}
...
static void bt_dev_show_info(struct bt_dev *hdev)
{
	int i;
	...
	if (IS_ENABLED(CONFIG_BT_LOG_SNIFFER_INFO)) {
		...
		sys_memcpy_swap(irk, hdev->irk[0], 16);
		LOG_INF("IRK%s: 0x%s", hdev->id_count > 1 ? "[0]" : "", bt_hex(irk, 16));
#endif
	}

	for (i = 1; i < hdev->id_count; i++) {
		LOG_INF("Identity[%d]: %s", i, bt_addr_le_str(&hdev->id_addr[i]));

		if (IS_ENABLED(CONFIG_BT_LOG_SNIFFER_INFO)) {
#if defined(CONFIG_BT_PRIVACY)
			uint8_t irk[16];

			sys_memcpy_swap(irk, hdev->irk[i], 16);
	...
	if (IS_ENABLED(CONFIG_BT_SMP) &&
		...
		bt_keys_foreach_type(hdev, BT_KEYS_ALL, bt_keys_show_sniffer_info, NULL);
	}

	LOG_INF("HCI: version %s (0x%02x) revision 0x%04x, manufacturer 0x%04x",
		bt_hci_get_ver_str(hdev->hci_version), hdev->hci_version, hdev->hci_revision,
		hdev->manufacturer);
	LOG_INF("LMP: version %s (0x%02x) subver 0x%04x", bt_hci_get_ver_str(hdev->lmp_version),
		hdev->lmp_version, hdev->lmp_subversion);
}
...
static void hci_vs_init(struct bt_dev *hdev)
{
	...
	if (IS_ENABLED(CONFIG_BT_HCI_VS_EXT_DETECT)) {
		...
		if ((hdev->hci_version < BT_HCI_VERSION_5_0) ||
		    bt_id_read_public_addr(hdev, &addr)) {
	...
	err = bt_hci_cmd_send_sync(hdev, BT_HCI_OP_VS_READ_VERSION_INFO, NULL, &rsp);
	if (err) {
	...
	err = bt_hci_cmd_send_sync(hdev, BT_HCI_OP_VS_READ_SUPPORTED_COMMANDS,
				   NULL, &rsp);
	if (err) {
	...
	if (BT_VS_CMD_SUP_FEAT(hdev->vs_commands)) {
		err = bt_hci_cmd_send_sync(hdev, BT_HCI_OP_VS_READ_SUPPORTED_FEATURES,
					   NULL, &rsp);
		if (err) {
		...
		memcpy(hdev->vs_features, rp.feat->features,
		       BT_DEV_VS_FEAT_MAX);
		net_buf_unref(rsp);
	}
}
...
#else
	if (hdev->drv->setup) {
		err = hdev->drv->setup(&setup_params);
		if (err) {
			return err;
		}
	}
...
static void rx_queue_put(struct bt_dev *hdev, struct net_buf *buf)
{
	...
	const int err = k_work_submit(&hdev->rx_work);
#elif defined(CONFIG_BT_RECV_WORKQ_BT)
	const int err = k_work_submit_to_queue(&bt_workq, &hdev->rx_work);
...
static int bt_init(struct bt_dev *hdev)
{
	int err;
...
#if defined(CONFIG_BT_ISO)
void bt_testing_set_iso_mtu_mc(uint8_t dev_id, uint16_t mtu)
{
	struct bt_dev *hdev;
...
int bt_enable_mc(uint8_t dev_id, bt_ready_cb_t cb)
{
	...
	if (IS_ENABLED(CONFIG_BT_SETTINGS)) {
		...
		err = bt_set_name_mc(dev_id, CONFIG_BT_DEVICE_NAME);
		if (err) {
...
#if defined(CONFIG_BT_RECV_WORKQ_BT)
	/* RX thread */
	if (bt_workq.flags & K_WORK_QUEUE_STARTED) {
		LOG_DBG("Work queue already started");
	} else {
...
int bt_disable_mc(uint8_t dev_id)
{
...
#if defined(CONFIG_BT_PER_ADV_SYNC)
	bt_periodic_sync_disable(hdev);
#endif /* CONFIG_BT_PER_ADV_SYNC */

#if defined(CONFIG_BT_CONN)
	if (IS_ENABLED(CONFIG_BT_SMP)) {
		bt_pub_key_hci_disrupted(hdev);
	}
	...
	bt_addr_le_copy(&hdev->random_addr, BT_ADDR_LE_ANY);
...
bool bt_is_ready_mc(uint8_t dev_id)
{
	...
	return atomic_test_bit(hdev->flags, BT_DEV_READY);
}
...
int bt_set_name_mc(uint8_t dev_id, const char *name)
{
	...
	if (!strcmp(hdev->name, name)) {
		return 0;
	}

	memcpy(hdev->name, name, len);
	hdev->name[len] = '\0';
	...
	if (IS_ENABLED(CONFIG_BT_CLASSIC) &&
		...
		err = bt_br_write_local_name_mc(dev_id, name);
		if (err) {
			LOG_WRN("Unable to set BR/EDR name");
			return err;
		}

		return bt_br_write_ext_inq_response_mc(dev_id, true);
	}
...
#if defined(CONFIG_BT_DEVICE_APPEARANCE_DYNAMIC)
int bt_set_appearance_mc(uint8_t dev_id, uint16_t appearance)
{
	struct bt_dev *hdev;
...
int bt_le_filter_accept_list_add_mc(uint8_t dev_id, const bt_addr_le_t *addr)
{
	...
	err = bt_hci_cmd_send_sync(hdev, BT_HCI_OP_LE_ADD_DEV_TO_FAL, buf, NULL);
	if (err) {
...
int bt_le_filter_accept_list_remove_mc(uint8_t dev_id, const bt_addr_le_t *addr)
{
	...
	err = bt_hci_cmd_send_sync(hdev, BT_HCI_OP_LE_REM_DEV_FROM_FAL, buf, NULL);
	if (err) {
...
int bt_le_filter_accept_list_clear_mc(uint8_t dev_id)
{
	...
	err = bt_hci_cmd_send_sync(hdev, BT_HCI_OP_LE_CLEAR_FAL, NULL, NULL);
	if (err) {
...
int bt_le_set_chan_map_mc(uint8_t dev_id, uint8_t chan_map[5])
{
	...
	if (!BT_CMD_TEST(hdev->supported_commands, 27, 3)) {
	...
	return bt_hci_cmd_send_sync(hdev, BT_HCI_OP_LE_SET_HOST_CHAN_CLASSIF,
				    buf, NULL);
}
...
int bt_le_set_rpa_timeout_mc(uint8_t dev_id, uint16_t new_rpa_timeout)
{
	...
	if (new_rpa_timeout == hdev->rpa_timeout) {
		return 0;
	}

	hdev->rpa_timeout = new_rpa_timeout;
	atomic_set_bit(hdev->flags, BT_DEV_RPA_TIMEOUT_CHANGED);
			   ...
			   const uint8_t *vs_config)
{
	...
	err = bt_hci_cmd_send_sync(hdev, BT_HCI_OP_CONFIGURE_DATA_PATH, buf, &rsp);
	if (err) {
		return err;
	}
...
static void tx_work_handler(struct k_work *work)
{
	struct bt_dev *hdev = CONTAINER_OF(work, struct bt_dev, tx_work);

	LOG_DBG("dev:%d, TX process start", hdev->dev_id);
	if (process_pending_cmd(hdev, K_NO_WAIT)) {
	...
	if (IS_ENABLED(CONFIG_BT_CONN_TX)) {
		bt_conn_tx_processor(hdev);
	}
}

void bt_tx_irq_raise(struct bt_dev *hdev)
{
	LOG_DBG("dev:%d, kick TX", hdev->dev_id);
	k_work_submit(&hdev->tx_work);
}